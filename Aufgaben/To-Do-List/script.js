...
*/
// select the elements
var list = document.getElementById("list"); // Wo die Items sind
var input = document.getElementById("input"); //Was der User eintippt 
// classes names
...
*/
var LIST, id;
var data = localStorage.getItem("TODO"); // item wird vom lokalen speicher aufgerufen 
...
function addToDo(toDo, id, done, trash) {
    if (trash) {
        return;
    } //wenn item in trash dann wird der code darunter nicht ausgeführt (return=stoppen)
    var DONE = done ? CHECK : UNCHECK; // if todo is fertig then use CHECK, else (also nicht fertig) use UNCHECK
    var LINE = done ? LINE_THROUGH : ""; // if todo fertig then use LINE_TROUGH, else mach nichts
    ...
    var total = document.querySelector("#summe");
    total.innerHTML = LIST.length;
}
...
list.addEventListener("click", function (event) {
    var element = event.target; //return clicked element inside list
    var elementJob = element.attributes.job.value; //fertig oder löschen
